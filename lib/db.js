...
export async function saveGameToHistory(userId, gameId, puzzleData, score, mistakes, completionTime = null) {
  try {
    console.log('🎯 REAL SAVE - Starting database insert:', {
      userId,
      gameId,
      score,
      mistakes,
    ...
    console.log('من امیدم');

    const result = await sql`
      INSERT INTO game_history (
        user_id, game_id, puzzle_title, puzzle_size, score, mistakes, completion_time, created_at
      ) 
      VALUES (
        ${userId}, 
        ${gameId}, 
    ...
    console.log('🎯 REAL SAVE - Database insert result:', result);
    
    if (result && result.length > 0) {
      console.log('✅ REAL SAVE - Successfully saved with ID:', result[0].id, 'Score:', result[0].score);
      return result[0];
    } else {
      console.log('❌ REAL SAVE - No result returned from insert');
      return null;
    }
    
  } catch (error) {
    console.error('❌ REAL SAVE - Database error:');
