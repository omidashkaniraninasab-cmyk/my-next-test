...
export async function saveGameToHistory(userId, gameId, puzzleData, score, mistakes, completionTime = null) {
  try {
    console.log('📝 saveGameToHistory DB function called:', {
      userId,
      gameId,
      score,
      mistakes,
    ...
    const result = await sql`
      ...
      VALUES (
        ${userId}, ${gameId}, ${puzzleData?.title || 'جدول روزانه'}, 
        ${puzzleData?.size || 6}, ${score}, ${mistakes}, ${completionTime}
      )
      RETURNING *
    `;

    console.log('✅ Game history saved to database:', result[0]);
    return result[0];
    
  } catch (error) {
    console.error('❌ Error in saveGameToHistory DB function:', error);
    throw error;
  }
}
