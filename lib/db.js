...
export async function createNewGame(userId, puzzleData) {
  try {
    ...
    console.log('🔍 Puzzle data type:', typeof puzzleData);
    console.log('🔍 Puzzle data keys:', Object.keys(puzzleData || {}));
    
    // 🆕 بررسی و validate کردن داده‌ها
    if (!puzzleData) {
      throw new Error('Puzzle data is required');
    }
    
    // 🆕 تبدیل userId به number برای اطمینان
    ...
    // 🆕 ساده‌سازی داده‌ها برای جلوگیری از مشکلات JSON
    const simplifiedPuzzle = {
      title: puzzleData.title || 'جدول روزانه',
      size: puzzleData.size || 6,
      grid: puzzleData.grid,
      solution: puzzleData.solution || [],
      across: puzzleData.across || {},
      down: puzzleData.down || {}
    };

    console.log('🔍 Simplified puzzle ready for insert');
    ...
    const result = await sql`
      ...
      VALUES (
        ${numericUserId},
        ${JSON.stringify(simplifiedPuzzle)}::jsonb,
        ${JSON.stringify({
          userInput: [], 
          cellStatus: [], 
          selectedCell: [0,0],
          first_input_recorded: false
        })}::jsonb,
        CURRENT_TIMESTAMP
      )
      RETURNING *
    ...
    console.log('✅ New game created, ID:', result[0]?.id);
    return result[0];
  } catch (error) {
...
export async function saveGameToHistory(userId, gameId, puzzleData, score, mistakes, completionTime = null) {
  try {
    console.log('🎯 Saving game history:', { userId, gameId, score, mistakes });
...
// تابع برای ریست وضعیت بازی روزانه
...
// اضافه کردن ستون today_game_completed
...
// علامت‌گذاری شروع بازی و افزایش شمارش فقط یک بار برای هر بازی
