...
export async function createUserProfileTable() {
  await sql`
    CREATE TABLE IF NOT EXISTS user_profiles (
      ...
      today_game_completed BOOLEAN DEFAULT FALSE  -- فیلد جدید
    )
  `;
}
  ...
  try {
    ...
    const result = await sql`
      ...
      SET 
        ...
        today_game_completed = FALSE,  -- بازی رو برای فردا آزاد کن
        last_score_reset_date = ${today}
      WHERE last_score_reset_date IS NULL OR last_score_reset_date < ${today}
    `;
    
    console.log(`✅ Daily scores and game status reset for ${result.count} users`);
    return result.count;
  } catch (error) {
