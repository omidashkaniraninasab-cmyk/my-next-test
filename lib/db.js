...
// در lib/db.js اضافه کن
export async function resetDailyGameStatus() {
  try {
    const result = await sql`
      UPDATE user_profiles 
      SET today_game_completed = FALSE
      WHERE today_game_completed = TRUE
    `;
    console.log(`✅ Daily game status reset for ${result.count} users`);
    return result.count;
  } catch (error) {
    console.error('Error resetting daily game status:', error);
    throw error;
  }
}